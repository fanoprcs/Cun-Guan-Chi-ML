...
def no_media_show_pointByhand(img, point1, point2, point3, grid_number, whether_grid, hand_type):
    ...
    try:
        originx, originy, xEnd_x, xEnd_y = zuobiao(img, detect)
    except:
        return 0
    ...
    point1Xdir_x, point1Xdir_y = getPoint(originx, originy, xdir_m,   point1_xvalue, detect, False)
    point1_x, point1_y = getPoint(point1Xdir_x, point1Xdir_y, ydir_m,  point1_yvalue, detect, True)
    point2Xdir_x, point2Xdir_y = getPoint(originx, originy, xdir_m,   point2_xvalue, detect, False)
    point2_x, point2_y = getPoint(point2Xdir_x, point2Xdir_y, ydir_m,  point2_yvalue, detect, True)
    point3Xdir_x, point3Xdir_y = getPoint(originx, originy, xdir_m,   point3_xvalue, detect, False)
    point3_x, point3_y = getPoint(point3Xdir_x, point3Xdir_y, ydir_m,  point3_yvalue, detect, True)
    ####################################print################################################
    ...
    if whether_grid == True:
        ...
        for i in range(xaxis_grid):
            xgrid_point[i][0], xgrid_point[i][1] = getPoint(originx, originy, xdir_m, (i/xaxis_grid  * xdirValue), detect, False)
        for i in range(yaxis_grid):
            ygrid_point[i][0], ygrid_point[i][1] = getPoint(originx, originy, ydir_m, (i/yaxis_grid * ydirValue), detect, True)
        ...
        for i in range(xaxis_grid):
            another_xdir_point[i][0], another_xdir_point[i][1] = getPoint(xgrid_point[i][0], xgrid_point[i][1], ydir_m, ydirValue, detect, True)
        for i in range(yaxis_grid):
            another_ydir_point[i][0], another_ydir_point[i][1] = getPoint(ygrid_point[i][0], ygrid_point[i][1], xdir_m, xdirValue, detect, False)
        for i in range(xaxis_grid):
